...
4. The *polynomial model* has an ACF with polynomial decay:
  $
...
the polynomial model cannot be represented exactly by any finite-dimensional linear SDE.
Instead, the polynomially decaying ACF can be represented as a superposition of exponentials,
...
$
Using this identity,
the polynomial kernel can be rewritten as
$
...
Finally, trajectories of the polynomial model are generated by sampling a finite set of independent exponential kernels,
and taking linear combinations of the resulting trajectories according to
$
  x^("poly")(t) = sum_i sqrt(w(tau_i)) space x_i^"exp" (t)
$
