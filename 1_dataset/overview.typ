...
$
  ...
  &= integral_0^t integral_0^t e^(bold(Theta)(t-s))2 bold(B) e^(bold(Theta^dagger)(t-s')) dif s dif s'
$
In Zwanzig's derivation,
the equilibrium second moment $M$ is obtained as the solution of this integral reformulated as a continuous-time Lyapunov equation
$
  bold(Theta) bold(M) + bold(M) bold(Theta)^dagger & = 2 integral_0^infinity dv(, t) e^(t bold(Theta)) bold(B) e^(t bold(Theta)^dagger) dif t \
  &= 2 (e^(t bold(Theta)) bold(B) e^(t bold(Theta)^dagger))_(t arrow.r infinity) - 2 bold(B)\
  &= - 2 bold(B)
$
...
$
  bold(Theta) bold(M)_h + bold(M)_h bold(Theta)^dagger & = 2 integral_0^(h) dv(, t) e^(t bold(Theta)) bold(B) e^(t bold(Theta)^dagger) dif t \
  &= 2 e^(h bold(Theta)) bold(B) e^(h bold(Theta)^dagger) - 2 bold(B)\
$
...
$
  bold(X) (t + h) ~ cal(N)(e^(h bold(Theta)) bold(X)(t), bold(M)_h)
$
...
$
$
  x(t+1) ~ cal(N)(e^(theta)x(t), sigma^2) ,quad sigma^2 = A_0/(2tau) (1 - e^(-(2h)/tau))
$
...
The increment covariance $bold(M_h)$ is obtained by solving the finite-time Lyapunov equation as described above.
